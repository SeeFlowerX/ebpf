...
func newPerfEventRing(cpu, perCPUBuffer, watermark int, overwritable, unwind_stack, regs bool) (*perfEventRing, error) {
	if watermark >= perCPUBuffer {
		return nil, errors.New("watermark must be smaller than perCPUBuffer")
	}

	fd, err := createPerfEvent(cpu, watermark, overwritable, unwind_stack, regs)
	if err != nil {
		return nil, err
	}
...
func createPerfEvent(cpu, watermark int, overwritable, unwind_stack, regs bool) (int, error) {
	if watermark == 0 {
		watermark = 1
	}
